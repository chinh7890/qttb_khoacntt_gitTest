...
class thongkemaymocmodel extends My_model {
	...
	{
		parent::__construct();
		$this->table='thongkemaymoc';
	}
	public function getAlldata(){
		$this->db->select('*');
		$data= $this->db->get('thongkemaymoc');
		$data=$data->result_array();
		return $data;
	}

    public function laynamthongke(){
        $query = 'SELECT namthongke FROM `thongkemaymoc` GROUP BY namthongke';
        return $this->db->query($query)->result_array();
    }

    public function laydonvicu($idphongkho){
        $query = 'SELECT DISTINCT tendonvi, maphong FROM phong_kho pk, donvi dv, thongkemaymoc tb WHERE pk.madonvi = dv.id AND tb.maphongkho = pk.id AND tb.id='. $idphongkho;
	...
	{
		$query = 'SELECT maso FROM thongkemaymoc WHERE model="'.$model.'" ORDER BY id DESC LIMIT 1';
		return $this->db->query($query)->result_array();
	}

	public function themdanhsachthongkemaymoc($data){
        $this->db->db_debug = false;

        if(!@$this->db->insert('thongkemaymoc',$data))
        {
            $error = $this->db->error();
            return false;
        }else{
    ...
    public function layphongkhobangten($iddv){
        $query = 'SELECT pk.id,pk.maphong FROM phong_kho pk WHERE madonvi = "'. $iddv.'"';
        return $this->db->query($query)->result_array();
    }
	...
	public function laydulieu($maloai,$manhom, $madonvi){
		$this->db->select('tb.id,
							...
							pk.tenphong
							');
        $this->db->from('thongkemaymoc tb, 
        	phong_kho pk, 
        	donvi dv,
        	loaithongkemaymoc loaitb,
        	nhomthongkemaymoc nhomtb,
        	tinhtrangthietbi tt
        	');
	...
	{
		$query = "SELECT DISTINCT *,COUNT(tentb) as soluong FROM thongkemaymoc tb,phong_kho pk, donvi dv WHERE pk.madonvi = dv.id AND tb.maphongkho = pk.id AND dv.id = ".$madonvi." AND tb.namthongke ='".$namthongke."' group by tentb, maphongkho,tinhtrang,chatluong";
		return $this->db->query($query)->result_array();
	}

    public function ketso($nam)
    {
        $query = "INSERT INTO thongkemaymoc(tentb, maso, mota, namsd, nguongoc, donvitinh, soluong, tontai, gia, chatluong, ghichu, model, tinhtrang, matinhtrang, maphongkho, maloai, manhom, idthietbi, namthongke) 
            SELECT tentb, maso, mota, namsd, nguongoc, donvitinh, soluong, tontai, gia, chatluong, ghichu, model, tinhtrang, matinhtrang, maphongkho, maloai, manhom, id as idthietbi, '".$nam."' as namthongke FROM maymocthietbi
            ";
        $this->db->query($query);
    }
	...
	//pagination
	var $table = "thongkemaymoc";  
    var $order_column = array("id", "tentb", "mota", "maso","namsd","nguongoc","donvitinh","gia","maphongkho","chatluong","ghichu","model","tinhtrang");  

    function make_condition($donvi, $tenloai, $tennhom, $nguongoc, $gia, $tinhtrang,$maphong, $nam, $namsd, $chatluong){
    	if($donvi != NULL)
        {
    ...
    function make_query($donvi, $tenloai, $tennhom, $nguongoc, $gia, $tinhtrang,$maphong, $nam, $namsd, $chatluong)  
     {  
   		$this->db->select('tb.id,
					tb.tentb,
					tb.mota,
        ...
        $this->make_condition($donvi, $tenloai, $tennhom, $nguongoc, $gia, $tinhtrang,$maphong, $nam, $namsd, $chatluong);
      ...
      function make_datatables($donvi, $tenloai, $tennhom, $nguongoc, $gia, $tinhtrang,$maphong, $nam, $namsd, $chatluong){  
           $this->make_query($donvi, $tenloai, $tennhom,$nguongoc, $gia, $tinhtrang,$maphong, $nam, $namsd, $chatluong);  
           if($_POST["length"] != -1)  
           {  
      ...
      function get_filtered_data($donvi, $tenloai, $tennhom, $nguongoc, $gia, $tinhtrang,$maphong, $nam, $namsd, $chatluong){  
           $this->make_query($donvi, $tenloai, $tennhom, $nguongoc, $gia, $tinhtrang,$maphong, $nam, $namsd, $chatluong);  

           $query = $this->db->get();  
           return $query->num_rows();  
      }       
      function get_all_data($donvi, $tenloai, $tennhom, $nguongoc, $gia, $tinhtrang,$maphong, $nam, $namsd, $chatluong)  
      {  
        $this->db->select('tb.id,
			tb.tentb,
			tb.mota,
